...
export function describePolicyNormalPath(options: {
    ...
    const syncPolicy = String(options.syncPolicy || 'metadata_only').trim().toLowerCase();
    ...
    return syncPolicy === 'full_sync'
        ...
        : 'Active | workspace sync is metadata_only; machine defaults remain local';
}
...
export function describeDataPolicyScope(options: {
    ...
    if (preset === 'review') {
        return 'Workspace sync stays metadata_only for this workspace. Machine capture and debug defaults come from the Review machine preset.';
    }
    if (preset === 'debug') {
        return 'Workspace sync stays metadata_only for this workspace. Machine capture and debug defaults come from the Debug machine preset.';
    }
    if (preset === 'lean') {
        return 'Workspace sync stays metadata_only for this workspace. Machine capture and debug defaults come from the Lean machine preset.';
    }
