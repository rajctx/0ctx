...
import { ALL_SUPPORTED_CLIENTS, DEFAULT_HOOK_INSTALL_CLIENTS, DEFAULT_MCP_CLIENTS, SESSION_START_AGENTS, isGaHookAgent, parseClients, parseEnableMcpClients, parseHookClients, validateExplicitPreviewSelection } from '../cli-core/clients';
...
export function createCliRegistry() {
    ...
    const { commandHook: commandConnectorHook } = createHookCommands({
        resolveRepoRoot, parseOptionalStringFlag, resolveContextIdForHookIngest: (repoRoot, explicitContextId) => resolveContextIdForHookIngest(repoRoot, explicitContextId), validateExplicitPreviewSelection, parseHookClients: raw => parseHookClients(raw), installHooks, readHookInstallState, parsePositiveIntegerFlag, getHookDumpRetentionDays, pruneHookDumps, extractSupportedHookAgent, readStdinPayload, normalizeHookPayload, resolveHookTranscriptPath, resolveCodexSessionArchivePath, readCodexArchiveCapture, readTranscriptCapture, readCodexCapture, readInlineHookCapture, persistHookTranscriptSnapshot, persistHookTranscriptHistory, appendHookEventLog, persistHookDump, resolveHookCaptureRoot, validateHookIngestWorkspace, buildHookCaptureMeta, ensureChatSessionNode, ensureChatCommitNode, asRecord, safeGitValue, sendToDaemon
    });
