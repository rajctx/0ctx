...
import { listWorkstreamInsightsRecord, promoteInsightNodeRecord } from './graph/insights';
...
export class Graph {
    ...
    private knowledgeSessionDeps() { return buildKnowledgeSessionDeps({ getSessionDetail: (contextId, sessionId) => this.getSessionDetail(contextId, sessionId), getByKey: (contextId, key, options) => this.getByKey(contextId, key, options), addNode: (params) => this.addNode(params), ensureEdge: (fromId, toId, relation) => ensureEdgeRecord(this.db, fromId, toId, relation) }); }
    ...
    previewKnowledgeFromSession(contextId: string, sessionId: string, options: { checkpointId?: string | null; maxNodes?: number; source?: 'session' | 'checkpoint'; minConfidence?: number } = {}): KnowledgePreviewResult { return previewKnowledgeFromSessionRecord(this.knowledgeSessionDeps(), contextId, sessionId, options); }
    extractKnowledgeFromSession(contextId: string, sessionId: string, options: { checkpointId?: string | null; maxNodes?: number; source?: 'session' | 'checkpoint'; allowedKeys?: string[] | null; minConfidence?: number } = {}): KnowledgeExtractionResult { return extractKnowledgeFromSessionRecord(this.knowledgeSessionDeps(), contextId, sessionId, options); }
    previewKnowledgeFromCheckpoint(checkpointId: string, options: { maxNodes?: number; minConfidence?: number } = {}): KnowledgePreviewResult { return previewKnowledgeFromCheckpointRecord(this.knowledgeCheckpointDeps(), checkpointId, options); }
    extractKnowledgeFromCheckpoint(checkpointId: string, options: { maxNodes?: number; allowedKeys?: string[] | null; minConfidence?: number } = {}): KnowledgeExtractionResult { return extractKnowledgeFromCheckpointRecord(this.knowledgeCheckpointDeps(), checkpointId, options); }
