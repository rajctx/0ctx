...
export async function getGraphData(
  ...
  try {
    const res = await bffPost<GraphPayload>('/api/v1/runtime/command', {
      method: 'getGraphData',
      contextId,
...
export async function listChatSessionsAction(
  ...
  if (!contextId) return [];
  const res = await bffPost<ChatSessionSummary[]>('/api/v1/runtime/command', {
...
export async function listChatTurnsAction(
  contextId: string,
  sessionId: string,
  machineId?: string | null
): Promise<ChatTurnSummary[]> {
  if (!contextId || !sessionId) return [];
  const res = await bffPost<ChatTurnSummary[]>('/api/v1/runtime/command', {
...
export async function getNodePayloadAction(nodeId: string, machineId?: string | null): Promise<NodePayloadRecord | null> {
  ...
  const res = await bffPost<NodePayloadRecord>('/api/v1/runtime/command', {
...
export async function getHookHealthAction(machineId?: string | null): Promise<HookHealthSnapshot | null> {
  const res = await bffPost<HookHealthSnapshot>('/api/v1/runtime/command', {
  ...
  return {
    ...
    agents: Array.isArray(res.data.agents)
      ? res.data.agents.map(agent => ({
        ...
        status: agent.status === 'Supported' || agent.status === 'Planned' || agent.status === 'Skipped'
          ? agent.status
          : 'Skipped',
