(function (factory) {
  ...
  function describeDesktopPolicyHint(options) {
    ...
    if (preset === 'shared' || syncPolicy === 'full_sync') {
      return 'This workspace is explicitly opted into full sync. Machine retention and debug settings remain local machine defaults.';
    }
    if (options.actionHint) {
      return `Lean is the normal default. Workspace sync stays metadata-only unless you opt this workspace into full sync. ${options.actionHint}`;
    }
    return 'Lean is the normal default. Review and Debug only change machine capture defaults. Full sync is a separate workspace override.';
  }
